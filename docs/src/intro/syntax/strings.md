...
`Whamm` มีการรองรับสตริงแบบพื้นฐานมาก สตริงจะถูกฉีดเข้าไปในโปรแกรม Wasm ที่ทำ instrumentation
และแทนด้วยทูเพิล: `(memory_address, length)` โดยกำหนดได้เฉพาะสตริงสแตติก (static string)
เช่น `"i'm a string"` เท่านั้น ยังไม่รองรับการสร้างสตริงแบบพลวัต เช่น `"i " + " am " + " groot"`
อย่างไรก็ตาม หากต้องการฟังก์ชันนี้ ก็ใช้วิธีหลบเลี่ยงด้วยการดึงสตริงจากไลบรารีของผู้ใช้ได้
...
สตริงสามารถใช้ตัวอักษร `\` เพื่อหลบลำดับอักขระบางอย่างได้ เช่น:
- `\n` ขึ้นบรรทัดใหม่
- `\t` แท็บ
...
ยูทิลิตี้ต่อไปนี้มีให้ใช้สำหรับการดำเนินการกับสตริง:
...
สตริงสามารถส่งต่อไปยัง / ดึงกลับจากไลบรารีได้ผ่านการดำเนินการกับหน่วยความจำ เพื่อให้
สิ่งนี้ทำงานได้ในไลบรารีของคุณเอง คุณต้องเปิดเผยฟังก์ชันสำหรับจัดสรร/คืนหน่วยความจำ
