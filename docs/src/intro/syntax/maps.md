...
`Whamm` มีแมป (map) สำหรับจัดเก็บคู่คีย์-ค่า
ซึ่งคล้ายกับชนิด `Map` ของ `java` และ `dict` ของ `python`
อันที่จริงมันคือชนิด `HashMap` ของ `Rust` เป๊ะๆ ... เพราะ `Whamm` นำชนิดของ `Rust` นี้มาใช้อยู่เบื้องหลัง!
