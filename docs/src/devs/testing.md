...
[ไฟล์ `.wast`](https://webassembly.js.org/docs/contrib-wat-vs-wast.html#:~:text=WAST%20is%20a%20superset%20of,easier%20to%20write%20by%20hand) ถูกใช้เพื่อการทดสอบและช่วยให้นักพัฒนาเขียนเทสต์ได้ง่ายขึ้น
เราใช้ไฟล์ `.wast` เพื่อเข้ารหัสแอสเซอร์ชันที่ควรผ่านเมื่อรันโมดูล `wasm` ที่ทำ instrumentation แล้ว
...
เพื่อยืนยันว่า*แอสเซอร์ชันทั้งหมด**ล้มเหลว**ก่อนทำ instrumentation* ไฟล์ `.wast` ใหม่ 5 ไฟล์จะถูกสร้างขึ้นด้วยโมดูลต้นฉบับและรันบนอินเทอร์พรีเตอร์ที่กำหนดค่าไว้
และเพื่อยืนยันว่า*แอสเซอร์ชันทั้งหมด**ผ่าน**หลังทำ instrumentation* ไฟล์ `.wast` ใหม่ 2 ไฟล์จะถูกสร้างขึ้น หนึ่งไฟล์ต่อสคริปต์ `whamm` ที่ระบุ โดยรวมแอสเซอร์ชันไว้ใต้สคริปต์ `whamm` ที่เกี่ยวข้อง
...
2. ใช้คอมเมนต์ระบุสคริปต์ `Whamm` ไวยากรณ์: `;; WHAMM --> <whamm_script>`
   ...
   - ถ้ามี `asserts` หลายอันใต้คอมเมนต์ `Whamm` อันหนึ่ง พวกมันทั้งหมดจะถูกรันกับ `module` ที่ทำ instrumentation แล้วซึ่งได้จากสคริปต์ `Whamm` นั้น
3. แอสเซิร์ตทั้งหมดควร **ล้มเหลว** หากถูกรันโดยไม่มีการทำ instrumentation
...
1. **แยกส่วนประกอบของเทสต์** จากแต่ละไฟล์ `.wast` ที่พบใต้ `tests/wast_suite` เป็น `WastTestCase` แต่ละรายการ
   - โมดูล `wasm`
   - สคริปต์ `whamm`
   - รายการแอสเซอร์ชันที่*ควรเป็นจริง*หลังทำ instrumentation
2. **ยืนยันว่าแอสเซอร์ชันทั้งหมดล้มเหลวก่อนทำ instrumentation** ด้วย `whamm`
   ...
   - เราสร้างไฟล์ `.wast` ใหม่แบบนี้เพราะอินเทอร์พรีเตอร์ที่เราใช้จะออกเมื่อเจอแอสเซอร์ชันแรกที่ล้มเหลวต่อไฟล์ `.wast` แต่เราต้องการรับประกันคุณสมบัตินี้*กับทุกแอสเซอร์ชัน*
